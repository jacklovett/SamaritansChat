...
export class ChatComponent implements OnInit, OnDestroy {
  ...
  private handleNewMessage(msgResponse: any) {
    this.chatMessages.push(msgResponse);
    if (
      this.activeChat &&
      (this.activeChat.username === msgResponse.sender ||
        this.activeChat.username === msgResponse.recipient)
    ) {
      this.displayMessages.push(msgResponse);
    } else {
      // increases number of unread messages for relevant chat
      for (let i = 0; i < this.chatUsers.length; i++) {
        if (msgResponse.sender === this.chatUsers[i].username) {
